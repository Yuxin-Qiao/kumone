...
(function () {
  ...
  function showToast(msg) {
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.kumoneBridge) {
      window.webkit.messageHandlers.kumoneBridge.postMessage({ action: 'toast', message: msg });
    }
  ...
  // Global hooks for native platform callbacks (Android & iOS)
  window.onNativePlaybackProgress = onNativePlaybackProgress;
  window.onNativePlaybackComplete = onNativePlaybackComplete;
  window.onNativeRemotePlay = () => resumeAudio();
  window.onNativeRemotePause = () => pauseAudio();
  window.onNativeRemoteNext = () => playNextTrack(true);
  window.onNativeRemotePrev = () => playPrevTrack();
  window.onNativeRemoteSeek = (posMs) => seekAudio(posMs);
  ...
  function playAudio(url, track, startPosMs = 0) {
    ...
    } else if (window.AndroidBridge && typeof window.AndroidBridge.playAudio === 'function') {
  ...
  function pauseAudio() {
    ...
    updatePlayPauseButtons();
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.kumoneBridge) {
      window.webkit.messageHandlers.kumoneBridge.postMessage({ action: 'pauseAudio' });
    } else if (window.AndroidBridge && typeof window.AndroidBridge.pauseAudio === 'function') {
  ...
  function resumeAudio() {
    ...
    updatePlayPauseButtons();
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.kumoneBridge) {
      window.webkit.messageHandlers.kumoneBridge.postMessage({ action: 'resumeAudio' });
    } else if (window.AndroidBridge && typeof window.AndroidBridge.resumeAudio === 'function') {
  ...
  function seekAudio(posMs) {
    ...
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.kumoneBridge) {
      window.webkit.messageHandlers.kumoneBridge.postMessage({ action: 'seekAudio', position: posMs });
    } else if (window.AndroidBridge && typeof window.AndroidBridge.seekAudio === 'function') {
  ...
  function renderSettingsView() {
    ...
    document.getElementById('btn-open-repo').onclick = () => {
      const repoUrl = 'https://github.com/Yuxin-Qiao/kumone';
      if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.kumoneBridge) {
        window.webkit.messageHandlers.kumoneBridge.postMessage({ action: 'openExternal', url: repoUrl });
      } else if (window.AndroidBridge && typeof window.AndroidBridge.openExternal === 'function') {
        window.AndroidBridge.openExternal(repoUrl);
      } else {
  ...
  function doJumpNetease() {
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.kumoneBridge) {
      window.webkit.messageHandlers.kumoneBridge.postMessage({ action: 'openNeteaseApp', unikey: state.currentUnikey });
  ...
  function setupEventListeners() {
    ...
    el.actionCopyLink.onclick = () => {
      if (state.actionTrack) {
        ...
        if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.kumoneBridge) {
          window.webkit.messageHandlers.kumoneBridge.postMessage({ action: 'copyToClipboard', text: url });
          showToast('已复制歌曲链接');
        } else if (window.AndroidBridge && typeof window.AndroidBridge.copyToClipboard === 'function') {
  ...
  window.kumoneApp = {
    ...
    async e2e(cmd, arg) {
      ...
      switch (cmd) {
        case 'tab': {
          const tab = document.querySelector('.nav-tab[data-tab="' + value + '"]');
          if (tab) tab.click();
          break;
        }
        case 'search': {
          ...
          setTimeout(() => {
            const input = document.getElementById('search-keyword-input');
            if (!input) return;
            input.value = value || '晴天';
            state.searchKeyword = input.value;
            performSearch(state.searchKeyword, 1);
          }, 900);
          break;
        }
        case 'login':
          ...
          break;
        case 'loginTab': {
          const loginTab = document.querySelector('.login-tab-btn[data-login-tab="' + value + '"]');
        ...
        case 'openPlayer':
          el.bottomPlayerBar.click();
          break;
        case 'closePlayer':
          if (el.fsBtnClose) el.fsBtnClose.click();
          break;
        case 'playlist': {
          const card = document.querySelector('[data-action="open-playlist"]');
          if (card) card.click();
          break;
        }
        case 'daily': {
          const daily = document.getElementById('btn-hero-daily');
